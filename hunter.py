...
class Hunter:
    ...
    def __init__(self, dirname, arch, ext) -> None:
    ...
    def _run_idahunt(self, user_args):
        ...
        log_operation('Executing {}'.format(args))
    ...
    def run_script(self, script):
        resolved = pathlib.Path(script).resolve()
        if resolved.exists():
            # IDA Hunt prefers fully qualified paths.
            script = str(resolved)

        return self._run_idahunt(['--filter', fr"filters\names.py -a {self.arch} -e {self.ext} -v", '--scripts', script])
